...
public class XDrive {
    ...
    void encoderDrive(double speed, double diagonalRightCM, double diagonalLeftCM, double headingTarget, double TimeoutS) {

        final double COUNTS_PER_CM = 19.8059; // (1120 / 2) / (9 * 3.1415)
        final double gain = 0.3; //TODO: find a good heading gain value
        ...
        while(opmode.opModeIsActive() && (autonTime.seconds() < TimeoutS) && (flDrive.isBusy() || rlDrive.isBusy() || rrDrive.isBusy())) {
            ...
            double error = headingSetpoint - imu.getHeadingRadians();
            double steering = error * gain;
            flDrive.setPower(Math.abs(diagonalRightSpeed) + steering);
            rlDrive.setPower(Math.abs(diagonalLeftSpeed) + steering);
            rrDrive.setPower(Math.abs(diagonalRightSpeed) - steering);
            frDrive.setPower(Math.abs(diagonalLeftSpeed) - steering);
        }
